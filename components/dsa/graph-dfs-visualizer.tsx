...
import type { GraphNode, GraphEdge } from "@/lib/algorithms/graph-bfs-steps" // Reuse types
import { type DFSStep, generateDFSSteps } from "@/lib/algorithms/graph-dfs-steps"
...
// Using same sample graph as BFS for consistency
...
export default function GraphDFSVisualizer() {
  ...
  const [animationSteps, setAnimationSteps] = useState<DFSStep[]>([])
  ...
  const displayedDescriptions = useMemo(() => {
    ...
    return animationSteps.slice(0, currentStepIndex + 1).map((step, index) => ({
      id: `desc-dfs-${index}-${step.description.slice(0, 10).replace(/\s/g, "_")}`,
      text: step.description + (step.stack.length > 0 ? `\nStack: [${step.stack.join(" -> ")}]` : ""),
      lineNumber: index + 1,
    }))
  ...
  const handleStartDFS = () => {
    ...
    const steps = generateDFSSteps(graphNodes, graphEdges, startNodeId)
  ...
  return (
    <div className="flex flex-col items-center p-6 space-y-6 bg-slate-800 text-white rounded-lg shadow-2xl w-full max-w-4xl mx-auto">
      <h2 className="text-3xl font-bold text-fuchsia-400">Graph Traversal (DFS)</h2>
      ...
      <div className="w-full space-y-1">
        <div className="flex justify-between items-center px-1">
          ...
          <div className="flex items-center space-x-2">
            <Button
              ...
              className="text-fuchsia-400 hover:text-fuchsia-300 px-2 py-1 h-auto"
        ...
        >
            ...
            <div key={desc.id} className="flex items-start mb-1">
              ...
              <p
                className={cn(
                  ...
                  index === displayedDescriptions.length - 1 ? "font-semibold text-fuchsia-300" : "text-slate-400",
                )}
              >
                {desc.text}
              </p>
          ...
          {displayedDescriptions.length === 0 && (
            <p className="text-sm text-slate-500 italic">Log is empty. Start DFS.</p>
          )}
        </div>
      </div>
      ...
      <svg width={SVG_WIDTH} height={SVG_HEIGHT} className="bg-slate-900/50 rounded-md border border-slate-700">
        <AnimatePresence>
          {graphEdges.map((edge) => {
            ...
            const isDiscoveryEdge = currentStep?.path[toNode.id] === fromNode.id && currentStep?.visited.has(toNode.id)
            const isCurrentPathEdge =
              currentStep?.currentNodeId === fromNode.id &&
              currentStep?.currentNeighborId === toNode.id &&
              !currentStep?.visited.has(toNode.id)
            return (
              <motion.line
                key={edge.id}
                x1={fromNode.x}
                ...
                x2={toNode.x}
                y2={toNode.y}
                stroke={isCurrentPathEdge ? "#fbbf24" : isDiscoveryEdge ? "#a21caf" : "#4b5563"} // amber-400, fuchsia-600, gray-600
                strokeWidth={isCurrentPathEdge || isDiscoveryEdge ? "3" : "2"}
                initial={{ opacity: 0.5 }}
                animate={{ opacity: 1 }}
              ...
              <circle
                ...
                fill={
                  currentStep?.currentNodeId === node.id
                    ? "#f59e0b" // amber-500 (current)
                    : currentStep?.stack.includes(node.id)
                      ? "#d946ef" // fuchsia-500 (in stack)
                      : currentStep?.visited.has(node.id)
                        ? "#a21caf" // fuchsia-600 (visited)
                        : "#374151" // gray-700
                }
                stroke="#9ca3af"
                strokeWidth="2"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 w-full pt-4 border-t border-slate-700">
        <Button
          ...
          variant="outline"
          className="text-fuchsia-400 border-fuchsia-400 hover:bg-fuchsia-400 hover:text-slate-900"
        >
          <StepBack className="mr-2 h-4 w-4" /> Prev
        </Button>
        <Button onClick={handlePlayPause} variant="default" className="bg-fuchsia-500 hover:bg-fuchsia-600">
        ...
        >
          <RotateCcw className="mr-2 h-4 w-4" /> Reset DFS
        </Button>
        <div className="flex flex-col items-center space-y-1 col-span-2 sm:col-span-4 md:col-span-2 md:col-start-3">
          ...
          <Slider
            ...
            disabled={isPlaying}
            className="w-full [&>span:first-child]:h-1 [&>span:first-child>span]:bg-fuchsia-400 [&>span:nth-child(2)>span]:bg-fuchsia-400"
          />
        </div>
      </div>
