...
import { type QuickSortStep, generateQuickSortSteps } from "@/lib/algorithms/quick-sort-steps"
import { cn } from "@/lib/utils"

const INITIAL_ARRAY = [7, 2, 1, 6, 8, 5, 3, 4, 9]
...
export default function QuickSortVisualizer() {
  ...
  const [animationSteps, setAnimationSteps] = useState<QuickSortStep[]>([])
  ...
  useEffect(() => {
    setAnimationSteps(generateQuickSortSteps(arrayData))
  ...
  const displayedDescriptions = useMemo(() => {
    return animationSteps.slice(0, currentStepIndex + 1).map((step, index) => ({
      id: `desc-qui-${index}-${step.description.slice(0, 10).replace(/\s/g, "_")}`,
  ...
  // ... (handleNextStep, handlePrevStep, handleReset, handlePlayPause, handleSpeedChange, handleRandomize are similar) ...
  ...
  const getBarColor = (index: number, step: QuickSortStep | undefined) => {
    ...
    if (index === step.pivotIndex) return "bg-red-500" // Pivot
    if (step.partitionRange) {
      if (index >= step.partitionRange[0] && index <= step.partitionRange[1]) {
        if (index === step.leftPointer) return "bg-blue-500" // i pointer
        if (index === step.rightPointer) return "bg-yellow-400" // j pointer (scanner)
        return "bg-purple-400" // Element within current partition range
      }
    }
    return "bg-slate-600"
  }
  ...
  return (
    <div className="flex flex-col items-center p-6 space-y-6 bg-slate-800 text-white rounded-lg shadow-2xl w-full max-w-3xl mx-auto">
      <h2 className="text-3xl font-bold text-red-400">Quick Sort Visualization</h2>
            ...
            <div key={desc.id} className="flex items-start mb-1">
              ...
              <p
                className={cn(
                  ...
                  index === displayedDescriptions.length - 1 ? "font-semibold text-red-300" : "text-slate-400",
                )}
              >
                {desc.text}
              </p>
        ...
        {currentStep.array.map((element, index) => (
          <motion.div
            ...
            className={cn(
              ...
              index === currentStep.pivotIndex ? "ring-2 ring-red-300" : "",
              index === currentStep.leftPointer ? "ring-1 ring-blue-300" : "",
              index === currentStep.rightPointer ? "ring-1 ring-yellow-200" : "",
            )}
          >
            {element.value}
          </motion.div>
        ))}
        {/* Partition range indicator (optional) */}
        {currentStep.partitionRange && (
          <div
            className="absolute bottom-0 h-full border-l-2 border-r-2 border-purple-500/50 pointer-events-none"
            style={{
              left: `${currentStep.partitionRange[0] * (40 + 4)}px`, // 40px bar width + 4px gap
              width: `${(currentStep.partitionRange[1] - currentStep.partitionRange[0] + 1) * (40 + 4) - 4}px`,
            }}
          >
            <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-xs text-purple-400">Partition</span>
          </div>
        )}
      </div>
      {/* Controls */}
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 w-full pt-4 border-t border-slate-700">
        ...
        <Button onClick={handlePlayPause} variant="default" className="bg-red-500 hover:bg-red-600">
        ...
        <div className="flex flex-col items-center space-y-1 col-span-2 sm:col-span-3 md:col-span-1">
          ...
          <Slider
            ...
            disabled={isPlaying}
            className="w-full [&>span:first-child]:h-1 [&>span:first-child>span]:bg-red-400 [&>span:nth-child(2)>span]:bg-red-400"
          />
        </div>
      </div>
